...
func TestCode_Unwrap(t *testing.T) {
	...
	e2 := fmt.Errorf("something wrong1: %w", e1)
	e3 := fmt.Errorf("something wrong2: %w", e2)

	assert.True(t, errors.Is(e3, e1))

	e4 := errors.Unwrap(e3)
	assert.Equal(t, e4, e2)
	assert.True(t, errors.Is(e4, e1))
	e5 := errors.Unwrap(e4)
	assert.Equal(t, e5, e1)
}

func TestTopCode(t *testing.T) {
	n0 := Error(20010, "err0")
	e1 := fmt.Errorf("x: %w", n0)
	e2 := fmt.Errorf("x: %w", e1)
	e3 := fmt.Errorf("x: %w", e2)
